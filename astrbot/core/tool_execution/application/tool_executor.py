...
class ToolExecutor:
    ...
    async def execute(
        ...
        **tool_args
        ...
        timeout_enabled = self._should_enable_timeout(
            run_context.tool_call_timeout,
            tool.name
        )
        
        async for result in self._execute_with_timeout(
            tool, run_context, handler, method_name,
            timeout_enabled, **tool_args
        ):
            yield result
    ...
    def _should_enable_timeout(self, timeout: float, tool_name: str) -> bool:
        """判断是否启用超时"""
        return timeout > 0 and tool_name not in self.BACKGROUND_TOOL_NAMES

    
    async def _execute_with_timeout(
        self, tool, run_context, handler, method_name,
        timeout_enabled, **tool_args
    ) -> AsyncGenerator[mcp.types.CallToolResult, None]:
        """带超时控制的执行"""
        ...
        while True:
            try:
                ...
                processed = await self.result_processor.process(resp)
                if processed:
                    yield processed

            
            except asyncio.TimeoutError:
                ctx = self._build_timeout_context(
                    tool, run_context, handler, tool_args
                )
